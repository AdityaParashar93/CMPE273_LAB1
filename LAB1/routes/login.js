...
exports.checkLogin = function(req,res){
	...
	var getUser="select * from userinfo where cx_email='"+req.param("username")+"'";
	...
	mysql.fetchData(function(err,results){
		...
		{
			if(results.length > 0){
				if(bcrypt.compareSync(req.param("password"),results[0].cx_password)){
				console.log("Result Element:"+(results[0].cx_display));
				console.log("valid Login");
				logger.log('info', "Query:: " + getUser);
				req.session.display_name=results[0].cx_display;
